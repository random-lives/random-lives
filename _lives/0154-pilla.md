---
layout: life
title: "Pilla"
birth_year: "46 BC"
birth_year_numeric: -45
death_year: "11 AD"
age_at_death: 55
birth_date: "July 4, 46 BC"
death_date: "April 6, 11 AD"
location: "Virupapuram, Halaharvi, Kurnool, Andhra Pradesh, India"
country: "India"
latitude: 15.4
longitude: 77.11
map_url: "https://www.google.com/maps/place/15.4,77.11/@15.4,77.11,5z"
old_lifestyle: "Rural"
lifestyle: "Farmer"
era: "Holocene"
sex: "F"
age_tag: "Elder (50+)"
continent: "Asia"

# Debug information (not displayed on page):
# Age at death: 55
# Agreeableness (% rank): 49
# Birth date: July 4, 46 BC
# Birth year: 46 BC
# Birthplace altitude: 448
# Birthplace annual precipitation: 616.0
# Birthplace biome: Deserts & Xeric Shrublands
# Birthplace coldest month temp: 18.18000030517578
# Birthplace cropland percentage: 1.8
# Birthplace ecotype: Deccan thorn scrub forests
# Birthplace irrigated_not_rice percentage: 0.0
# Birthplace irrigated_rice percentage: 0.0
# Birthplace latitude: 15.4
# Birthplace longitude: 77.11
# Birthplace map url: https://www.google.com/maps/place/15.4,77.11/@15.4,77.11,5z
# Birthplace mean temp: 27.103042602539062
# Birthplace modern address: Virupapuram, Halaharvi, Kurnool, Andhra Pradesh, India
# Birthplace modern country: India
# Birthplace modern region: Andhra Pradesh
# Birthplace pasture percentage: 0.0
# Birthplace population density: 9.75
# Birthplace rainfed_not_rice percentage: 1.8
# Birthplace rainfed_rice percentage: 0.0
# Birthplace rangeland percentage: 0.21
# Birthplace urban_area percentage: 0.0
# Birthplace warmest month temp: 37.13999938964844
# Conscientiousness (% rank): 31
# Death date: April 6, 11 AD
# Era: Holocene
# Extraversion (% rank): 70
# Handedness: right
# Height (% rank): 39
# Honesty-Humility (% rank): 84
# Intelligence (% rank): 88
# Lifestyle: Rural
# Name: Pilla
# Naming category: inferable
# Neuroticism (% rank): 92
# Openness (% rank): 6
# Physical attractiveness (% rank): 66
# Sex: F
# Sexual orientation: heterosexual
# adult_standing: 'Somewhat lower than birth: widowed/financially strained smallholder,
#   more dependent on kin/patrons'
# birth_order_position: 4
# cause_of_death: Acute infectious fever (malaria/undifferentiated febrile illness)
# ethnicity: Deccan Dravidian (Andhra/proto-Telugu agrarian communities)
# father_occupation: Self-cultivating small farmer (dryland crops; primary livelihood
#   from own plot)
# historical_context:
# - event: Expansion of inland trade networks connecting the Deccan to western coastal
#     ports (via passes and market towns), increasing circulation of coins and opportunities/pressures
#     to pay obligations in cash rather than only grain.
#   timing: adolescence to late adulthood (c. 30 BC–11 AD)
# - event: Endemic malaria and other 'fever' ecologies around seasonal standing water
#     and irrigation tanks/streams, with occasional unusually severe fever years affecting
#     labor availability and mortality.
#   timing: recurrent, especially during/after monsoon seasons throughout life
# household_social_status: Smallholding cultivators (modest land, mostly subsistence;
#   limited surplus)
# household_structure_at_birth: Nuclear household (parents married, living independently
#   with no co-resident grandparents/uncles)
# household_structure_at_death: Lived with her widowed mother or elderly parent(s) (she
#   as caregiver/co-dependent)
# language: Dravidian + some liturgical Sanskrit formulae (minimal, non-fluent)
# literacy: Neither read nor write
# marital_status: Widowed at death (husband died earlier; she did not remarry or was
#   not currently remarried)
# mental_disorder: No clear modern disorder (high-strung/anxious temperament but subclinical;
#   functional most of the time)
# migration: Stayed in/near birth settlement (same village or within a few km) for most
#   of life
# mother_occupation: Household/domestic work plus regular farm labor on the family plot
#   (weeding, harvesting, processing grain)
# narrative_plan:
#   caretakers:
#   - death_year: 10
#     name: Rakki
#     narrative_role: Primary caregiver and farm partner; deeply anxious after repeated
#       infant deaths, protective and ritual-minded. Relationship with Pilla is close
#       but tense—warmth mixed with worry and constant caution.
#     role: mother
#   - death_year: -12
#     name: Kodan
#     narrative_role: 'Self-cultivating small farmer; practical and socially connected
#       in the village. Relationship: affectionate but demanding; he expects Pilla to
#       be useful early and distrusts novelty.'
#     role: father
#   - death_year: -30
#     name: Periya
#     narrative_role: Appears in early childhood scenes around illness and rituals;
#       a calming counterweight to Rakki’s fearfulness and a source of informal knowledge.
#     role: maternal aunt (mother's sister, occasional helper)
#   children: []
#   incident_placements:
#   - age: 38
#     connection: Triggered by Korra’s death (age ~36-37) and followed by 2–4 years
#       of bad monsoons; borrowing seed grain/cash leads to pledged harvest and partial
#       loss of land-use security, pushing Pilla to co-reside with Rakki for survival.
#     incident: severe_economic_crisis
#   life_phases:
#   - age_range: 0-7
#     key_events:
#     - Born after three neonatal deaths; naming and protective rites are emphasized,
#       with frequent small offerings at the hearth and village boundary stones.
#     - Learns field-edge tasks early (scaring birds, carrying water, picking fuelwood)
#       as the household tries to rebuild confidence after losses.
#     - 'First vivid encounter with seasonal fever: a monsoon-time illness in the hamlet
#       leads to heightened household precautions and taboo observances.'
#     phase: Early childhood
#   - age_range: 8-15
#     key_events:
#     - Becomes a highly social helper in women’s work groups (threshing, winnowing),
#       talkative and quick at practical problem-solving despite anxiety.
#     - Exposure to cash/market pressures increases as traders and tax collection practices
#       become more visible; she hears more about coin payments and debts.
#     - Betrothal negotiations shaped by her being the only surviving child; family
#       seeks a nearby match to keep support close.
#     phase: Later childhood and adolescence
#   - age_range: 16-35
#     key_events:
#     - Marriage to Korra; settles into routine of dryland cultivation, grain processing,
#       and household rites with occasional Sanskrit auspicious phrases at festivals/life-cycle
#       rituals.
#     - Repeated pregnancies fail to result in live births (miscarriages/stillbirths/very
#       early deaths), reinforcing fear and ritual responses rather than open experimentation
#       with new remedies.
#     - Ongoing monsoon variability leads to alternating years of adequate harvest and
#       tight years; she learns careful storage and barter strategies.
#     phase: Early adult married life
#   - age_range: 36-45
#     key_events:
#     - Husband Korra dies (likely fever season); immediate loss of labor and bargaining
#       power with creditors and village officials.
#     - Sustained period of poor rains/crop shortfalls forces borrowing of seed grain
#       and cash; harvest is pledged and land-use rights become insecure.
#     - Moves (or effectively reintegrates) into her mother Rakki’s household nearby,
#       sharing caregiving and field management in reduced circumstances.
#     phase: Widowhood and economic decline
#   - age_range: 46-55
#     key_events:
#     - Lives with and cares for elderly widowed mother; relies more on kin networks
#       and reciprocity labor at peak seasons.
#     - Continues as a socially engaged village woman—present at work parties and rituals—while
#       remaining personally fearful about sickness and scarcity.
#     - Dies at 55 during an acute fever episode consistent with malaria/undifferentiated
#       febrile illness.
#     phase: Late adulthood and old age
#   other_named_characters:
#   - name: Soma
#     prominence: adolescence through late adulthood, especially in harvest seasons
#     relationship: Neighbor woman and work-group companion; occasional confidante and
#       gossip conduit
#   - name: Eruvan
#     prominence: widowhood and economic crisis (ages 36-42)
#     relationship: Better-off cultivator/moneylender figure who provides seed grain
#       or cash on pledge
#   - name: Nandi
#     prominence: childhood (protective rites) and during illness seasons
#     relationship: Village ritual specialist/elder who leads certain communal rites;
#       source of the Sanskrit auspicious phrases Pilla can repeat
#   - name: Ketta
#     prominence: adult life, particularly during drought or fodder scarcity years
#     relationship: Local herder who occasionally trades dung fuel/fodder info and carries
#       news of market prices
#   partners:
#   - name: Korra
#     narrative_role: Husband from the same local area; marriage is practical, work-centered,
#       and mostly steady though marked by Pilla’s worry and his occasional impatience.
#       His death (likely fever) triggers her economic crisis and long-term vulnerability.
#     relationship_end_year: -9
#     relationship_start_year: -30
#   siblings:
#   - birth_year: -49
#     death_age: 0
#     death_year: -49
#     name: Kanni
#     narrative_role: A first daughter remembered mostly through the mother’s grief;
#       her death becomes a reference point in household prayers and anxieties around
#       childbirth.
#     sex: F
#   - birth_year: -47
#     death_age: 0
#     death_year: -47
#     name: Ponni
#     narrative_role: Second infant loss; reinforces the mother’s reputation for 'unlucky
#       births' and shapes the cautious, ritual-heavy atmosphere around Pilla’s early
#       childhood.
#     sex: F
#   - birth_year: -46
#     death_age: 0
#     death_year: -46
#     name: Malla
#     narrative_role: A son who died at birth; his loss is felt as a blow to the household’s
#       future labor and standing, feeding later pressure on Pilla’s marriage prospects.
#     sex: M
#   trait_manifestations:
#   - consequence: Heightened ritual spending/time and strained interactions with practical-minded
#       kin; sometimes delays decisive action (e.g., selling a goat early) because she
#       fears making the wrong choice.
#     scene: After monsoon fevers spread, Pilla repeatedly checks sleeping mats, water
#       pots, and food stores, presses for extra boundary offerings, and catastrophizes
#       minor symptoms in herself and others.
#     timing: childhood recurring; peaks in widowhood (ages 36-42)
#     trait: Neuroticism (very high)
#   - consequence: Stability and social approval, but fewer adaptive experiments during
#       drought years; contributes to vulnerability during the economic crisis.
#     scene: She resists new cures or unfamiliar cultivation suggestions from a passing
#       trader or distant relative, preferring known seed varieties and customary rites
#       even when yields falter.
#     timing: adolescence through adulthood
#     trait: Openness (very low)
#   - consequence: Builds reciprocal support that later helps her and Rakki access help
#       in lean seasons; also exposes her to gossip pressure about childlessness.
#     scene: At threshing circles and water-collection trips she is a talker and organizer,
#       coordinating turn-taking and labor exchange, and she is often the one sent to
#       negotiate small swaps.
#     timing: adolescence through late adulthood
#     trait: Extraversion (high)
#   - consequence: Maintains reputation and some community trust, but limits her ability
#       to 'solve' crisis through opportunistic means; increases dependence on patrons/kin.
#     scene: When measuring out owed grain after borrowing seed, she insists on fair
#       measure even when the household is hungry, and she avoids covertly shifting
#       boundary stones.
#     timing: widowhood/economic crisis (ages 36-42)
#     trait: Honesty-Humility (high)
#   - consequence: Improves short-term coping (storage, barter, labor exchange), but
#       cannot fully overcome structural vulnerability of widowhood and small landholding.
#     scene: She quickly learns which buyers offer better exchange rates for surplus
#       millet and which household tools or animals can be spared without collapsing
#       production.
#     timing: early adult life into crisis years
#     trait: Intelligence (high)
#   - consequence: Occasional losses (spoilage, leaks) that compound scarcity; contributes
#       to feeling overwhelmed.
#     scene: During stress she sometimes postpones routine repairs (thatch, storage
#       bins) until problems become urgent, then works intensely in bursts.
#     timing: adult life, most visible during crisis
#     trait: Conscientiousness (somewhat low)
# number_of_children: '0'
# number_of_siblings: '3'
# occupation: Smallholder cultivator household laborer/manager (domestic production
#   + regular farm work on family plot)
# religion: Household and village rites centered on local deities/spirits (protective
#   offerings, fertility/rain rituals, ancestor observances; occasional Sanskrit auspicious
#   phrases)
# sibling_ages_at_death:
# - 0
# - 0
# - 0
# sibling_sexes:
# - F
# - F
# - M
# structured_incidents:
# - event: After her husband died, she struggled to keep the small dryland plot productive
#     through one or more bad monsoon seasons; with little surplus and no children to
#     add labor, the household fell into debt to a better-off cultivator or moneylender
#     and had to pledge part of the harvest (and likely a portion of land-use rights)
#     to get seed grain and meet dues.
#   reasoning: She is described as financially strained in adulthood and was widowed
#     and childless, which strongly increases the chance of debt, crop-failure-driven
#     distress, or partial loss of land/rights at least once.
#   tier: 1
#   timing: ages 36–42 (widowhood followed by a sustained 2–4 year period of crop shortfalls
#     and debt)
#   type: severe_economic_crisis
---

Pilla was born in a hamlet of dry fields and thorn scrub on the Deccan plateau, where the land broke into stony ridges and shallow soils and where families watched the sky for the monsoon. The Satavahana kings held power along the trade routes to the western coast, and their agents and local headmen collected grain and labor dues. Traders moved between inland market towns and the ports with salt, cloth, and stamped coins. Pilla's people spoke a Dravidian village speech and kept household and village rites for boundary spirits, ancestors, and fierce local goddesses, with a few Sanskrit blessing words that traveled with ritual specialists.

Her mother Rakki had already carried three pregnancies to the edge of birth and returned with empty arms. Kanni came first in 49 BC and died the day she arrived. Ponni followed in 47 BC and lasted no longer. In 46 BC a son, Malla, was born and died before his hair could be oiled. Rakki spoke of them in short phrases and counted them in offerings at the boundary stone. When Rakki's fourth child arrived in 45 BC, the women worked fast. They smeared the floor with fresh cow dung, set a small lamp at the hearth, and tied a black thread at the infant’s wrist. Rakki whispered a few words she had been taught—“svasti, svasti”—without knowing their grammar, only their use. They named the child Pilla, a small, plain name that fit a house that did not reach for grandeur.

Kodan, her father, farmed a small plot without irrigation. He sowed millets and pulses when the first steady rains softened the ground, and he kept a few animals for dung and milk. In the cool mornings Pilla followed him to the field edge, carrying a water pot no bigger than her forearm. He set her to scare birds with stones and a sling cord. Rakki kept her close, checking the child’s skin for heat and her stools for signs of sickness, pressing neighbors for remedies and then falling back on what she trusted: neem smoke, bitter leaves, and offerings at the village boundary stone.

Aunt Periya, Rakki’s sister, crossed from a nearby settlement when she could. She brought news, shared seed, and sat with Rakki when the fever season returned. When Pilla was five, after a heavy rain that left water standing in hoof prints and borrow pits, two households near them lost adults to a shaking illness. Kodan burned damp trash at the edge of the yard to keep insects down. Rakki insisted the family eat only hot food for days. Pilla watched, learned, and repeated the rules out loud.

By eight she moved in the women’s orbit. She helped winnow, tossing grain from a flat basket so the chaff drifted away, and she learned to judge when the wind was right. She talked while she worked. She liked working in a group and the small jokes that came with it—teasing over who could keep a clay pot balanced on the hip without a hand, or who stole the first roasted millet from the pan. Soma, a neighbor girl a few years older, taught her a song used for steadying the hands during pounding. Pilla carried stories between work groups, repeating them with extra details, then worrying afterward that she had said too much.

The market road mattered even in their hamlet. As Pilla grew, she heard more talk of coins: a tax collector who demanded payment in stamped metal for part of the dues, a trader who offered a better rate if grain came measured in a standard basket rather than a household scoop. Kodan disliked those changes. He trusted grain and labor and the rules he already knew. Pilla listened closely anyway. When she was thirteen, she walked with Soma to a nearby market day, carrying a small bundle of dried pulses. She watched how men weighed salt and how cloth was measured against a reed. She counted with her fingers and corrected Soma’s quick mental sums, then spent the walk home replaying the exchanges, worrying over whether they had been cheated.

Marriage negotiations started when she was fourteen. Kodan and Rakki pressed for a match nearby; they had no surviving sons, and they wanted family close enough to help in bad years. Pilla disliked the talk and asked the same questions repeatedly—who would her husband’s people be, what would she bring, where would she sleep. Kodan answered once and then told her to stop circling the matter. The match was set with Korra, a young man from a neighboring cluster of houses, with land like theirs and a father who worked his own hands.

Periya died that year, before the wedding, taken by a fever that came late in the season. Rakki sat with her sister's body and returned quieter than before. Pilla helped prepare food for the mourners and heard her mother repeat Periya's name at the boundary stone each morning for days.

Pilla married Korra at fifteen. Women rubbed turmeric on her arms, combed her hair tight, and fed her sweetened rice as a send-off. She walked to Korra's house with her head covered and her jaw clenched from holding back questions. Korra lived with his widowed mother, Velli, in a house like the one Pilla had left: packed-earth floor, thatch roof, a hearth in the corner. Korra had two brothers who had already married and built their own houses nearby. The first year settled into work. Pilla rose before full light, swept the yard, lit the hearth, and set gruel to cook. After sunrise she went to the field with Korra or with Velli, pulling weeds in lines, carrying bundles, and stacking stalks. She kept a habit of counting stores: how many baskets of millet remained, how many pots of pulse, how many coils of rope.

She wanted children and feared the attempt. A pregnancy ended early when she was seventeen; Rakki came over and sat with her, setting an offering of rice and flowers for a village goddess known for guarding mothers. When she was twenty, another pregnancy ended with a stillbirth. Pilla did not speak for days afterward except to give instructions for chores. She returned to pounding grain too soon and then lay awake, listening for sounds in the night yard. Korra did not strike her, but he showed his impatience in other ways: walking off mid-conversation, refusing to discuss seed choices, pushing her to attend festivals when she wanted to stay home. Velli told her to stop worrying so much, that worry itself could spoil a womb.

The years that followed brought more of the same. Pilla conceived again in her mid-twenties and lost the child before it quickened. She stopped counting after that. She worked the field, stored the grain, tended the hearth, and kept her fears to herself as much as she could. When a trader passing through suggested a new variety of pulse that grew faster in dry soil, she refused to try it. She trusted the seeds she knew. When Velli suggested a different remedy for Pilla's cramping, she shook her head and returned to the bitter leaves and neem smoke her mother had used. The known ways felt safer, even when they failed.

She remained tied to the women's groups. She could be sharp and funny with Soma, and she liked sitting near the threshing floor when the work was done, eating spiced lentils from a shared leaf. She saved the best pieces of roasted field rat for herself once and laughed at Soma's outrage, then spent the evening worrying that someone had noticed. When ritual days came, she laid a small line of rice at the threshold, touched water to her forehead, and murmured the little Sanskrit words she knew in the same breath as local-language requests to ancestors and boundary spirits.

Velli died in 17 BC. She had grown frail over several seasons, coughing through the cold months and eating less each year. Pilla washed her body and helped carry her to the burning ground. Korra said little. The house felt emptier.

Her father Kodan died in 12 BC, during a dry year that left the fields cracked. He had gone to the field as always, then sat down in the shade and did not get up. Pilla was thirty-three. She walked to her childhood home and found Rakki sitting outside, staring at nothing. The funeral rites took two days. Pilla helped measure the grain for offerings, then stayed while neighbors came and went. Rakki was alone now, and in the years that followed Pilla walked over more often, carrying a pot of gruel or a handful of dried pulse, sitting with her mother while the light faded.

Korra died in 9 BC. A fever moved through the settlement after the rains. He stopped going to the field, then stopped eating. Pilla sat near him with water and thin gruel, asked the ritual elder Nandi for a protective rite, and stayed awake through the night. Korra's body was carried out before sunrise.

She tried to hold the plot alone. She had no children to take up work, and Korra's two brothers—who had their own families to feed—pressed their claims on the land their father had once farmed. They took back a portion of the field that had come through their side. What remained was barely enough. The next monsoon failed in 7 BC. Seed sprouted unevenly, then dried. Pilla borrowed from Eruvan, a better-off cultivator who lent grain for repayment at harvest with interest counted in extra baskets. The year after that the rains came late again and pests took what survived. Eruvan demanded his due. Pilla measured out grain with careful hands, refusing to cheat the measure even when her own stores shrank to almost nothing. To keep seed for the next sowing, she pledged part of the harvest and accepted terms that meant Eruvan could claim first pick of whatever grew. Each year the debt rolled forward.

By 5 BC she moved back to her mother's house, only a short walk from where she had been born. Rakki was widowed and aging, and the two women shared what they had. They shared the field tasks they could still claim, relied on exchange labor at peak times, and took help from neighbors in return for food and future work promises. Pilla’s talkativeness served her then. She negotiated small swaps, passed messages, and kept the ties that brought a few extra hands at harvest. At the same time she worried openly, repeating worst outcomes until Rakki snapped at her to be quiet.

In her late forties and early fifties, Pilla spent more time close to home. She sorted grain, repaired baskets in uneven bursts, then left tasks half-finished until Rakki's scolding drove her back. She kept an eye on the sky and on the water pits after the rains, warning younger women not to let children sleep near standing water. When Rakki’s joints stiffened, Pilla washed her, cooked for her, and carried her outside to sit in the shade during the hottest part of the day. At festivals she still went, talking with Soma and others, touching the boundary stone with wet fingers, placing a pinch of rice and a smear of turmeric, and repeating “svasti” under her breath as she turned back toward the houses.

Rakki died in 10 AD. She had been failing for months, eating little, sleeping badly, and complaining of pains in her joints. Pilla sat with her through the last night and watched her breathing slow. She buried her mother with the rites she had seen performed for Kodan and Velli and Korra and all the others. Then she was alone.

Pilla remained in the same hamlet, still tied to the small field rights that had not been taken, living on the edge of other households' help. Soma brought her food sometimes. The next monsoon brought the familiar season of fevers. In 11 AD, during the weeks after the rains began, Pilla fell ill with a shaking fever and could not keep water down. Neighbors brought bitter decoctions and laid cool cloths on her skin, and Nandi marked a line of ash near the hearth. She died before the next market day.

Her body was washed, wrapped in cloth, and carried out beyond the settlement to the burning ground. A small offering of cooked rice and sesame was placed for her, and the fire was tended until the last embers settled.

<!--
**Issues found:**
- [Pass 1] 15-year gap (ages 17-33) with minimal content during married life
- [Pass 2] Korra's mother mentioned once then vanished; Korra's brothers appeared from nowhere
- [Pass 2] "co-dependent workers" jargon
- [Pass 5] Father Kodan's death was a single sentence, insufficiently developed
- [Pass 5] Aunt Periya's death mentioned in passing, rushed
- [Pass 7] Marriage and Periya's death jammed into same paragraph
- [Pass 8] Year + age redundancies ("in -30, at fifteen"; "in -9, when Pilla was thirty-six")
- [Pass 9] "Pilla's days narrowed" - vague paragraph opener
- [Pass 10] "Debt became a season of its own" - abstract metaphor
- [Pass 11] Political situation vague ("whichever distant power held the routes that year")
- [Pass 13] "The losses sat in the house like stored grain" - simile

**Changes made:**
- Added Satavahana kings to opening for historical grounding
- Rewrote simile about losses as concrete description of Rakki's behavior
- Split Periya's death into its own paragraph with Rakki's mourning
- Introduced Korra's household: widowed mother Velli, two married brothers with own houses
- Added new paragraph covering ages 20-33: third pregnancy loss, refusing new seeds/remedies, showing low-openness trait
- Added Velli's death in 17 BC as its own paragraph
- Expanded father Kodan's death with concrete details (how he died, funeral rites, Pilla's response)
- Explained Korra's brothers' land claims when they appear
- Replaced "co-dependent workers" with "shared what they had"
- Made debt mechanism concrete: "interest counted in extra baskets," "Eruvan could claim first pick"
- Fixed year + age redundancies throughout
- Replaced "Pilla's days narrowed" with concrete activities
- Replaced abstract "Debt became a season of its own" with "Each year the debt rolled forward"
- Added Soma bringing food at the end to maintain her presence
- Changed date formats from "-49" to "49 BC" throughout for consistency
-->
