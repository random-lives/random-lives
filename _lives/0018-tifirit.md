---
layout: life
title: "Tifirit"
birth_year: "824 BC"
birth_year_numeric: -823
death_year: "824 BC"
age_at_death: 0
birth_date: "February 3, 824 BC"
death_date: "February 3, 824 BC"
location: "Ternata ترناتة, Caïdat de Tinzouline قيادة تينزولين, Cercle de Zagora دائرة زاكورة, Drâa-Tafilalet ⴷⴰⵔⵄⴰ-ⵜⴰⴼⵉⵍⴰⵍⵜ درعة تافيلالت, Maroc ⵍⵎⵖⵔⵉⴱ المغرب"
country: "Morocco"
latitude: 30.4
longitude: -5.94
map_url: "https://www.google.com/maps/place/30.4,-5.94/@30.4,-5.94,5z"
lifestyle: "Rural"
era: "Holocene"
sex: "F"
age_tag: "Infant (0–1)"
continent: "Africa"

# Debug information (not displayed on page):
# Age at death: 0
# Birth date: February 3, 824 BC
# Birth year: 824 BC
# Birthplace altitude: 798
# Birthplace annual precipitation: 87.0
# Birthplace biome: Deserts & Xeric Shrublands
# Birthplace coldest month temp: 4.785999774932861
# Birthplace cropland percentage: 0.0
# Birthplace ecotype: North Saharan steppe and woodlands
# Birthplace irrigated_not_rice percentage: 0.0
# Birthplace irrigated_rice percentage: 0.0
# Birthplace latitude: 30.4
# Birthplace longitude: -5.94
# Birthplace map url: https://www.google.com/maps/place/30.4,-5.94/@30.4,-5.94,5z
# Birthplace mean temp: 22.254791259765625
# Birthplace modern address: Ternata ترناتة, Caïdat de Tinzouline قيادة تينزولين, Cercle
#   de Zagora دائرة زاكورة, Drâa-Tafilalet ⴷⴰⵔⵄⴰ-ⵜⴰⴼⵉⵍⴰⵍⵜ درعة تافيلالت, Maroc ⵍⵎⵖⵔⵉⴱ
#   المغرب
# Birthplace modern country: Morocco
# Birthplace modern region: Ouarzazate
# Birthplace pasture percentage: 0.0
# Birthplace population density: 1.149999976158142
# Birthplace rainfed_not_rice percentage: 0.0
# Birthplace rainfed_rice percentage: 0.0
# Birthplace rangeland percentage: 13.63
# Birthplace urban_area percentage: 0.0
# Birthplace warmest month temp: 41.784000396728516
# Death date: February 3, 824 BC
# Era: Holocene
# Lifestyle: Rural
# Name: Tifirit
# Naming category: inferable
# Sex: F
# birth_order_position: 5
# cause_of_death: Neonatal infection (acquired at/around delivery; rapid sepsis/tetanus-type
#   illness)
# ethnicity: Amazigh (Berber) local oasis/steppe agro‑pastoral communities of the pre-Roman
#   Maghreb
# father_occupation: Agro‑pastoralist (mixed herding + small-scale oasis cultivation/irrigation
#   work)
# household_social_status: Ordinary free household (commoner agro‑pastoralists; modest
#   land/water rights and small herds)
# household_structure_at_birth: Extended household with an elder relative (e.g., one/both
#   grandparents or widowed parent of either spouse) co-resident
# language: Berber (Amazigh) languages (Afroasiatic)
# mother_occupation: Household manager + subsistence labor (childcare, food processing,
#   water/fuel, textiles; helps with herding and oasis cultivation as needed)
# number_of_siblings: '4'
# religion: Syncretic influence from Punic/Phoenician religious practice (some Punic
#   deities/ritual forms alongside local rites), mostly via trade contacts
# sibling_ages_at_death:
# - 59
# - 1
# - 2
# - 24
# sibling_sexes:
# - F
# - M
# - F
# - M
---

Tifirit was born before sunrise in a mud-brick house near the cultivated strip of the Drâa valley, where Amazigh-speaking families lived by small herds and watered plots while distant coastal traders carried Phoenician goods along caravan routes. No king’s officers came to this place. Authority sat with elders, kin ties, and control of wells and channels.

Her father, Yidir, had slept in a cloak beside the courtyard wall. He rose to bring water from the nearest spring and to keep the goats quiet. He checked the narrow channel that fed their garden and pulled silt from a break in the bank with his hands. The household held more than his own small family. His widowed mother, Tinhinan, gave orders from a mat in the shade and counted out grain for the morning porridge.

Tifirit’s mother, Tasbirt, had carried four pregnancies before. Two children had died small; two lived. The older girl, Tiziri, ground barley on a stone, and the young man, Massin, took a tether and led animals to sparse grazing above the fields.

At the birth, Tasbirt’s sister brought a small lump of resin and a corded amulet that had come from traders passing south, marked with signs copied from Punic hands. She tied it near the baby’s cloth and poured a few drops of oil onto the threshold.

Tifirit cried and then quieted. By midday her skin turned hot and blotched. Tasbirt rinsed her with warmed water and pressed her to the breast again and again. Before the evening meal, Tifirit’s body stiffened. Tasbirt laid her down on folded wool, and Tifirit died there in the house.